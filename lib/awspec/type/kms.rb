module Awspec::Type
  class Kms < ResourceBase
    ...
    def has_principal_with_permission?(principal_to_match, action_regex = nil)
      retval = false
      res = kms_client.get_key_policy(key_id: id, policy_name: 'default')
      policy = JSON.parse(URI.decode(res.policy))
      policy["Statement"].each do |statement|
        if statement["Principal"]["AWS"].kind_of?(Array) then
          statement["Principal"]["AWS"].each do |principal|
            match = match_action(principal, statement["Action"], principal_to_match, action_regex)
            return true if match
          end
        else
          match = match_action(statement["Principal"]["AWS"], statement["Action"], principal_to_match, action_regex)
          return true if match
        end
      end
      return retval
